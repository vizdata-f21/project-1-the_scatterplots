...
resident on the y-axis, grouped by year. We mapped the mean median park size to 
both the size and alpha aesthetics because we believe these aesthetics visualized 
the difference in size in a meaningful way. Visualizing the change in size across 
time and regions using a size aeshetic allowed us to communicate the difference in
park size. This yielded a lineplot, that we 
noticed had similar features to the Napoleon plot. 
...
the difference in median park size across the quartiles. If we focus on the 
...
for spending, but in the 1st quartile for median park size. 
