...
To answer our first question (above), we used the "parks" dataset and analyzed 
...
### Approach
To answer our first question addressing the relationship between spending per 
resident and year, we first created a new variable, called "bins", to break down 
the spending per resident for each city into four quartiles (where the 1st 
quartile had the least spending per resident and the 4th quartile had the most).
This allowed us to analyze how spending trends shifted over time for each 
quartile. For this visualization, we created a line graph where each line 
represents a city and the color represents the quartile (or "bin") that the 
city falls into. The line graph allowed us to illustrate the trends in different 
cities' spending per resident over time (from 2012-2020) in what we felt was the
clearest way to demonstrate that relationship.
