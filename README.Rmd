...
For the line plot over time, we suspected there would be significant differences 
among regions, due to different natural geography, urban planning, and values 
...
spending between ~$170 and ~\$180 per resident, on average. There are few 
significant changes in mean median park size, except for some fluctuation within 
...
ggplot() +
  ...
  labs(x = NULL, y = NULL, size = "% of city\nthat is parkland", color = "ranking",
       title = "Top and bottom 10 city rankings of parks",
       subtitle = "scaled by % of city that is parkland") +
  coord_map() + 
  theme_void() + 
  theme(legend.position = c(.918,.275), legend.key.size = unit(0.45, "cm"),
...
a visually higher percentage of city land dedicated to parks than the 
lowest-ranked cities in the dataset. This conclusion is in line with our initial
hypothesis. We expected cities with a high overall score, calculated by the 
